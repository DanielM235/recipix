...
async function testFireflyConnection(config: ConnectorConfig): Promise<{ connected: boolean; message: string }> {
  try {
    ...
    if (error.response) {
      return { 
        connected: false, 
        message: `HTTP ${error.response.status}: ${error.response.data?.message || 'Connection failed'}` 
      }
...
async function testXeroConnection(config: ConnectorConfig): Promise<{ connected: boolean; message: string }> {
  // Placeholder for Xero integration
  return { connected: false, message: 'Xero integration not implemented yet' }
}

async function testERPConnection(config: ConnectorConfig): Promise<{ connected: boolean; message: string }> {
